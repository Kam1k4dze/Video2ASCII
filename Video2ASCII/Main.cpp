...
print(string name, float fps)
{
  ...
  for (int i = 0; i < out.size(); ++i) {
    ...
    // WriteConsoleA(stdOut, out[i].c_str(), strlen(out[i].c_str()), &written,
    // NULL);
    WriteConsoleOutputCharacter(
      hConsole, setname(out[i]), len, { 0, 0 }, &dwBytesWritten);
...
void
input_video(string filename, int cols, int rows)
{

  VideoCapture cap(filename.c_str(), cv::CAP_FFMPEG);
...
main(int argc, char** argv)
{
  ...
  if (argc == 4) {
    ...
    string filename2 =
      filename1.substr(0, filename1.find_last_of(".")) + ".wav";

    string ffmpeg = "ffmpeg -hide_banner -loglevel error -y -i \"" + filename1 +
                    "\" -ac 2 -f wav \"" + filename2 + "\"";
    cout << filename1 << endl << filename2 << endl << ffmpeg;
    // return 2;
    system(("mode con cols=" + string(argv[2]) + "lines=" + string(argv[3]))
             .c_str());
    system(ffmpeg.c_str());
